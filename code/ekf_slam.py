...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    Compute the output covariance: C_y = F_x * C_x * (F_x).T
    '''
    ...
    for i in range(k):
        beta, r = init_measure[i*2: i*2 + 2]
        landmark[i*2: i*2 + 2] = [x + r * np.cos(theta + beta), y + r * np.sin(theta + beta)]

        # Compute the Jacobian of the landmark function w.r.t the state (x, y, theta, beta, r)
        ...
        # Get the covariance matrix of shape (5, 5) for the state
        C = np.diag(np.hstack((init_pose_cov.diagonal(), init_measure_cov.diagonal())))
        # Compute the output covariance matrix of shape (2*k, 2*k) for the landmarks
        landmark_cov[i*2: i*2 + 2, i*2: i*2 + 2] = J.dot(C).dot(J.T)
...
def predict(X, P, control, control_cov, k):
    ...
    F = np.vstack((np.identity(3), np.zeros((2*k, 3))))  # shape (3 + 2k, 3)
    # Get the predicted state given the control, only the robot state is updated
    X = X + F.dot(np.vstack((d * np.cos(theta), d * np.sin(theta), alpha)))

    # compute Jacobian of the predicted robot pose w.r.t the state (x, y, theta)
    J = np.array([[1, 0, -d * np.sin(theta)],
                  [0, 1, d * np.cos(theta)],
                  [0, 0, 1]]).astype(float)
    G = F.dot(J).dot(F.T)
    np.fill_diagonal(G, 1)

    # Convert the noise distribution from robot coordinate into the world coordinate.
    H = np.array([[np.cos(theta), -np.sin(theta), 0],
                  [np.sin(theta), np.cos(theta), 0],
                  [0, 0, 1]]).astype(float)

    # Compute the predicted covariance, only the robot pose covariance is updated
    P = G.dot(P).dot(G.T) + F.dot(H).dot(control_cov).dot(H.T).dot(F.T)
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    x, y, theta = X_pre[:3]
    for i in range(k):
        delta_x, delta_y = X_pre[3 + i*2: 3 + i*2+2] - X_pre[:2]
        q = delta_x ** 2 + delta_y ** 2

        # Get the mask for Jacobian to get matrix of shape (2, 3 + 2k)
        F = np.zeros((5, 3 + 2*k))
        F[:3, :3] = np.identity(3)
        F[3:, 3 + i*2: 3 + i*2 + 2] = np.identity(2)
        # Compute Jacobian of measurement h w.r.t robot pose and landmark (x, y, theta, lx, ly)
        # Sensor data prediction for (beta, r)
        h = np.vstack((warp2pi(np.arctan2(delta_y, delta_x) - theta), np.sqrt(q)))
        ...
        H = H.dot(F).astype(float)

        # Get the Kalman gain of shape (3 + 2k, 2)
        K = P_pre.dot(H.T).dot(np.linalg.inv(H.dot(P_pre).dot(H.T) + measure_cov))
        X_pre += K.dot(measure[i * 2: i * 2 + 2] - h)
        P_pre = (np.identity(3 + 2*k) - K.dot(H)).dot(P_pre)
...
def evaluate(X, P, k):
    ...
    for i in range(k):
        l_pred = X[3:].T
        e = np.squeeze((l_true - l_pred))
        l_dist = e[i * 2: i * 2 + 2]
        euclidean_dist = np.linalg.norm(l_dist)
        mahalanobis_dist = np.sqrt(l_dist.dot(np.linalg.inv(P[3+i*2: 3+i*2+2, 3+i*2: 3+i*2+2])).dot(l_dist.T))
        print("Landmark {}/{}: Euclidean_dist: {}, Mahalanobis_dist: {}".format(i+1, k, euclidean_dist, mahalanobis_dist))
